...
RUN echo -n '\''bash /root/TRSS_Zhenxun/Main.sh "$@"'\''>/usr/local/bin/tszx &&\
    ...
    pacman -Syu --noconfirm --needed --overwrite "*" postgresql &&\
...
$Y- 正在启动 Docker 容器$O
"
docker run -itPd -h TRSS-Zhenxun --name TRSS_Zhenxun -v "$DIR":/root/TRSS_Zhenxun --restart=always trss:zhenxun||abort "Docker 容器启动失败，若要重装容器，请先停止并删除已安装容器"
echo -n "docker exec -it TRSS_Zhenxun bash /root/TRSS_Zhenxun/Main.sh "'"$@"'>"$CMDPATH/$CMD"&&chmod 755 "$CMDPATH/$CMD"||abort "脚本执行命令 $CMDPATH/$CMD 设置失败，手动执行命令：docker exec -it TRSS_Zhenxun bash /root/TRSS_Zhenxun/Main.sh"
echo "
