...
$Y- 正在启动 Docker 容器$O
"
docker run -itd -h TRSS-Zhenxun --name $DKNAME -v "$DIR":/root/TRSS_Zhenxun --restart always $([ $DKNAME = TRSS_Zhenxun ]&&echo "-p 8081:80 -p 8080:8080"||echo "-p 80 -p 8080") trss:zhenxun||abort "Docker 容器启动失败，若要重装容器，请先删除已安装容器，若要多开容器，请修改容器名"
