...
RUN pacman -Syu --noconfirm --needed --overwrite "*" postgresql nginx\
 && rm -rf /var/cache
RUN echo -n '\''exec bash /root/TRSS_Zhenxun/Main.sh "$@"'\''>/usr/local/bin/tszx\
...
$Y- 正在启动 Docker 容器$O
"
docker run -itd -h TRSS-Zhenxun --name $DKNAME -v "$DIR":/root/TRSS_Zhenxun --restart always $([ $DKNAME = TRSS_Zhenxun ]&&echo "-p 8080:8080 -p 8081:8081"||echo "-p 8080 -p 8081") trss:zhenxun||abort "Docker 容器启动失败，若要重装容器，请先删除已安装容器，若要多开容器，请修改容器名"
mkdir -vp "$CMDPATH"&&echo -n "exec docker exec -it $DKNAME bash /root/TRSS_Zhenxun/Main.sh "'"$@"'>"$CMDPATH/$CMD"&&chmod 755 "$CMDPATH/$CMD"||abort "脚本执行命令 $CMDPATH/$CMD 设置失败，手动执行命令：docker exec -it $DKNAME bash /root/TRSS_Zhenxun/Main.sh"
echo "
